...
- [x] **Global Search** — Command Palette (Cmd+K) now searches across customers, orders, and products via backend API with 300ms debounce. Results grouped by type (Customers/Orders/Products) above static nav items. Animated with motion/react: backdrop fade, dialog scale-in, staggered item slide-up, icon swap between Search/Loader. Keyboard navigation (arrow keys + Enter). All hardcoded notification badge counts removed from sidebar, icon rail, and user menu.
- [x] **Sidebar Hover Expand** — Sidebar starts collapsed (icon-only, 4rem wide). Expands on hover (200ms delay), collapses on mouse leave (400ms delay). Width transition via Intent UI CSS (`transition-[width] duration-300`). Text labels fade in with `motion.span` opacity animation. Logo swaps between icon and wordmark. Removed: PanelLeftClose button, SidebarTrigger from top nav, SidebarRail, IconRail component swap. Collapsed tooltips on all items. `SidebarProvider defaultOpen={false}`.
- [x] **Sidebar Polish** — Neon teal glow border on search trigger (`border-teal-500/20` + `shadow-[0_0_10px_rgba(20,184,166,0.08)]`). KokonutUI ProfileDropdown replaces old UserMenu in sidebar footer (Radix DropdownMenu, opens `side="top"`, teal gradient avatar ring, Settings/Notifications/Messages/Sign Out items, bending line indicator). Collapsed state shows avatar-only with tooltip. Header background unified: dark gradient moved from content div to SidebarInset so nav and content blend seamlessly.
