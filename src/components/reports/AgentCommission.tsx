...
export default function AgentCommission({ dateRange, filters }: { dateRange: ReportDateRange; filters?: ReportFilters }) {
      ...
      {agents.length > 0 && (
        <Card>
          <CardContent className="p-4">
            <h3 className="text-sm font-medium text-muted-foreground mb-3">Commission Earned by Agent</h3>
            <ChartContainer config={chartConfig} className="w-full" style={{ height: Math.min(260, Math.max(150, agents.length * 36)) }}>
              <BarChart data={agents} layout="vertical" margin={{ left: 80 }}>
                ...
                <Bar dataKey="commission_earned" radius={[0, 4, 4, 0]}>
                  {agents.map((_, i) => (
